...
mod tests {
    ...
    async fn event_repositories() {
        let pool = default_mysql_pool(TEST_CONNECTION_STRING).await;
        let id = uuid::Uuid::new_v4().to_string();
        let event_repo: MysqlEventRepository = MysqlEventRepository::new(pool.clone());
    ...
    async fn snapshot_repositories() {
        let pool = default_mysql_pool(TEST_CONNECTION_STRING).await;
        let id = uuid::Uuid::new_v4().to_string();
        let repo: MysqlEventRepository = MysqlEventRepository::new(pool.clone());
